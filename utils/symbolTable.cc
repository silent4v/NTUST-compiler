...
void SymbolTable::insert(std::string name, uint8_t type)
{
  ...
  if( std::any_of(scope.begin(), scope.end(), [&name](auto& var){ return var.first == name; }) )
  {
    std::cout << red("Fatal Error: ") << "`" + name + "`" << " has been declared.\n";
    std::exit(-1);
  }
...
{
  auto& scope = this->currentScope_();
  auto iter = std::find_if(scope.cbegin(), scope.cend(), [&name](auto& var){
    return var.first == name;
  });

  if( iter == scope.end() )
  {
    std::cout << red("Fatal Error: ") << "`" + name + "`" << " not found.\n";
    std::exit(-1);
  }
